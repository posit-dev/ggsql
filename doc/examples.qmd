...
WITH unordered_data AS (
    SELECT * FROM (VALUES
        ('2023-03-01'::DATE, 150.0),
        ('2023-01-01'::DATE, 100.0),
        ('2023-05-01'::DATE, 200.0),
        ('2023-02-01'::DATE, 120.0),
        ('2023-04-01'::DATE, 180.0)
    ) AS t(date, value)
)
VISUALISE
DRAW line MAPPING date AS x, value AS y FROM unordered_data ORDER BY date ASC
DRAW point MAPPING date AS x, value AS y, 'red' AS color FROM unordered_data SETTING size => 40
SCALE x SETTING type => 'date'
LABEL title => 'Line Chart with ORDER BY', x => 'Date', y => 'Value'
```
...
SELECT date, value, category FROM 'metrics.csv'
VISUALISE
DRAW line MAPPING date AS x, value AS y, category AS color FILTER category != 'Support' ORDER BY date ASC
DRAW point MAPPING date AS x, value AS y, category AS color SETTING size => 20 FILTER category != 'Support' ORDER BY date ASC
SCALE x SETTING type => 'date'
LABEL title => 'Sales and Marketing Metrics (Ordered)', x => 'Date', y => 'Value'
```
