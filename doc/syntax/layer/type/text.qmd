...
* `fontweight` Font weight, can be `'normal'` (default) or `'bold'`.
...
## Settings
* `offset_x` Horizontal offset expressed in absolute points.
* `offset_y` Vertical offset expressed in absolute points.
...
VISUALISE island AS x, n AS y
  ...
  DRAW text
    MAPPING n AS label
    SETTING vjust => 'top', offset_y => -11, fill => 'white'
```
...
VISUALISE island AS x, n AS y
  ...
  DRAW text
    MAPPING n AS label
    SETTING vjust => 'bottom', offset_y => 11
  SCALE y FROM [0, 200]
```
