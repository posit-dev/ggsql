...
* `align`: Determines the sides of the midline where the density is displayed. One of the following:
    * `'center'` or `'centre'` (default) displays a complete violin both sides of the midline.
...
To achieve this outcome, you can set the `align` setting and adjust `width` to taste.

```{ggsql}
VISUALISE Temp AS x, Month AS y FROM ggsql:airquality
DRAW violin SETTING width => 4, align => 'top'
SCALE ORDINAL y
```