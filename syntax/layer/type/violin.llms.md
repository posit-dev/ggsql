...
Violin plots display the distribution of a single continuous variable for multiple groups. The violins are mirrored kernel density estimates, similar to the [density](../../../syntax/layer/type/density.llms.md) layer, but organised as distinct groups.
...
- Primary axis (e.g. `x`): The categorical variable for grouping. If omitted a single violin is drawn for the whole distribution and the (one-tick) categorical axis is hidden.
...
A violin layer uses the same computation as a density layer. See the [density data transformation](../../../syntax/layer/type/density.llms.md#data-transformation) section for details. The major difference between a violin layer and a density layer is just the matter of display.
